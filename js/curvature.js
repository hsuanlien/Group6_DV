...
// -------------------- Legend --------------------
function drawCurvatureLegend(svg, geoPath) {
  const boxW = 300;
  const boxH = 110;
  const pad = 18;

  const [[x0, y0], [x1]] = geoPath.bounds({ type: "Sphere" });
...
async function renderMap(ctx) {
  ...
  drawCurvatureLegend(uiLayer, geoPath);
...
function renderScatter(svgEl, routes, mapSvg, routesSel) {
  ...
  const margin = { top: 26, right: 14, bottom: 34, left: 50 };
