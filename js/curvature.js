...
function drawCurvatureLegend(uiLayer, svgW)  {
  ...
  g.append("text")
    ...
    const legendItems = [
      { key: "equatorial", y: 35, label: `${CLASS_LABEL.equatorial} (≤${LAT_BINS.equatorialMax}°)` },
      { key: "mid",        y: 50, label: `${CLASS_LABEL.mid} (≤${LAT_BINS.midMax}°)` },
      { key: "polar",      y: 65, label: `${CLASS_LABEL.polar} (>${LAT_BINS.midMax}°)` },
    ];

    legendItems.forEach((it) => {
      g.append("line")
        .attr("x1", 12).attr("x2", 42)
...
async function renderMap(ctx) {
  ...
  const allCounts = countByDistanceClass(routesAll);
  console.log("[Curvature] routesAll (full filtered set):", {
    total: routesAll.length,
    ...allCounts
  });

  const routesCapped = capRoutes(routesAll, MAP_ROUTE_LIMIT, "longFirst");

  // const shownCounts = countByDistanceClass(routes);
  // console.log("[Curvature] routes shown on globe:", {
  //   total: routes.length,
  //   ...shownCounts
  // });
...
async function renderSide(ctx) {
  ...
  if (slot1) {
    ...
    slot1.style.alignItems = "stretch";
  }
  ...
  if (!svgEl2) {
    ...
    if (slot) {
      ...
      newSlot.className = "chart-slot";
      newSlot.style.height = "170px";
      newSlot.style.alignItems = "stretch";
      newSlot.style.marginTop = "12px";

      const newSvg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      newSvg.setAttribute("id", "curvature-chart-2");
      newSvg.style.width = "100%";
      newSvg.style.height = "100%";

      newSlot.appendChild(newSvg);
      slot.parentNode.insertBefore(newSlot, slot.nextSibling);
      svgEl2 = newSvg;
    }
  }
  ...
  // render chart 1
  d3.select(svgEl1).selectAll("*").remove();
  renderScatter(svgEl1, routes, ctx.mapSvg, routesSel);

  // render chart 2
  if (svgEl2) {
    d3.select(svgEl2).selectAll("*").remove();
    renderClassBreakdown(svgEl2, routes, routesSel);
  }
}

async function renderSide_origin(ctx) {
...
function countByCurvatureClass(routes) {
  const out = { equatorial: 0, mid: 0, polar: 0 };
  for (const r of routes) {
    if (!r?.cls) continue;
    if (out[r.cls] == null) out[r.cls] = 0;
    out[r.cls] += 1;
  }
  return out;
}

function renderClassBreakdown(svgEl, routes, routesSel) {
  ...
  svg.append("text")
    ...
    .attr("fill", "#374151")
    .attr("font-weight", 600)
    .text("Curvature class breakdown");

  const counts = countByCurvatureClass(routes);
  const total = Math.max(1, routes.length);

  const items = [
    { key: "equatorial", label: `${CLASS_LABEL.equatorial} (≤${LAT_BINS.equatorialMax}°)`, value: counts.equatorial },
    { key: "mid",        label: `${CLASS_LABEL.mid} (≤${LAT_BINS.midMax}°)`,              value: counts.mid },
    { key: "polar",      label: `${CLASS_LABEL.polar} (>${LAT_BINS.midMax}°)`,            value: counts.polar },
  ];
  ...
  // segments
  let acc = 0;
  const segs = svg.append("g").attr("class", "curv-class-bar");

  items.forEach((it) => {
    const segW = (it.value / total) * barW;
    segs.append("rect")
      .attr("x", barX + acc)
      .attr("y", barY)
      .attr("width", segW)
      ...
      .on("mouseenter", () => {
        ...
        routesSel
          .filter((d) => d.cls === it.key)
          .raise()
          .attr("stroke", CLASS_COLORS[it.key] || DEFAULT_ROUTE_COLOR)
          .attr("stroke-opacity", 0.95)
...
function countByDistanceClass(routes) {
  const counts = { short: 0, mid: 0, long: 0 };

  routes.forEach(r => {
    if (r.distance_class === "short") counts.short++;
    else if (r.distance_class === "mid") counts.mid++;
    else if (r.distance_class === "long") counts.long++;
  });
