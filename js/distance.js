...
const viewState = {
  ...
  airportsSel: null,      // d3 selection of airport dots
...
async function renderMap(ctx) {
  ...
  // Airports (endpoints): visual anchors; must not block route hover.
  const airportsSel = svg.append("g")
  ...
  viewState.airportsSel = airportsSel;
}
...
function applyBrushToMap() {
  const routesSel = viewState.routesSel;
  const airportsSel = viewState.airportsSel;
  if (!routesSel) return;

  const range = viewState.brushRangeKm;
  if (!range) {
    routesSel.style("display", null);
    if (airportsSel) airportsSel.style("display", null);
    return;
  }
  ...
  if (airportsSel) {
    ...
    for (const r of viewState.routesForView) {
      const km = r?.distance_km;
      if (!(km >= minKm && km <= maxKm)) continue;
      const a = r?.src;
      const b = r?.dst;
      const ka = a?.iata || a?.name;
      const kb = b?.iata || b?.name;
      if (ka) keysInRange.add(ka);
      if (kb) keysInRange.add(kb);
    }

    airportsSel.style("display", (d) => {
      const k = d?.iata || d?.name;
      return k && keysInRange.has(k) ? null : "none";
    });
  }
}
